...
type Limiter func(r *http.Request) error

// NoopLimiter will do nothing
type NoopLimiter struct{}

// Limit will do nothing
func (l *NoopLimiter) Limit(r *http.Request) error {
	return nil
}
...
func (l *ZeroRateLimiter) Limit(r *http.Request) error {
...
func (l *tokenBucketLimiter) Limit(r *http.Request) error {
	l.delegator.Wait(1)
	return nil
}
...
func (l *leakyBucketLimiter) Limit(r *http.Request) error {
	l.delegator.Take()
	return nil
}
...
func (set *optionSet) Wait(req *http.Request) (time.Duration, error) {
	now := time.Now()

	method := req.URL.Path

	limiter := set.getLimiter(method)
	if err := limiter(req); err != nil {
