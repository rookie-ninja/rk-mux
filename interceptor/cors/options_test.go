...
func TestNewOptionSet(t *testing.T) {
	// With empty option
	set := newOptionSet()
	assert.Equal(t, rkmuxinter.RpcEntryNameValue, set.EntryName)
	assert.Equal(t, rkmuxinter.RpcEntryTypeValue, set.EntryType)
	...
	set = newOptionSet(
		...
		WithSkipper(func(req *http.Request) bool {
			return true
		}),
