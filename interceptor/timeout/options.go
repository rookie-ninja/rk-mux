...
func (set *optionSet) Tick(req *http.Request, writer http.ResponseWriter, next http.Handler) error {
	var err error

	rk := set.getTimeoutRk(req.URL.Path)
	...
	bufPool := &bufferPool{}
	buffer := bufPool.Get()
	originalWriter := writer.(*rkmuxinter.RkResponseWriter).Writer
	newWriter := newWriter(originalWriter, buffer)

	// 3: assign new writer
	// writer must be RkResponseWriter
	writer.(*rkmuxinter.RkResponseWriter).Writer = newWriter
	...
	case recv := <-panicChan:
		...
		writer.(*rkmuxinter.RkResponseWriter).Writer = originalWriter
		panic(recv)
	...
	case <-timeoutChan:
		...
		rkmuxinter.WriteJson(writer, http.StatusRequestTimeout, rkerror.New(
...
func WithTimeoutAndResp(timeout time.Duration, resp http.HandlerFunc) Option {
...
// If response is nil, default globalResponse will be assigned
func WithTimeoutAndRespByPath(path string, timeout time.Duration, resp http.HandlerFunc) Option {
	return func(set *optionSet) {
