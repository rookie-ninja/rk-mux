...
const (
	...
	TvEntryDescription = "Internal RK entry which implements tv web with mux framework."
)
...
func readFileFromPkger(filePath string) []byte {
	...
	if file, err = pkger.Open(path.Join("github.com/rookie-ninja/rk-mux:/boot", filePath)); err != nil {
		return []byte{}
	}
...
func (entry *TvEntry) TV(writer http.ResponseWriter, req *http.Request) {
	...
	case "apis":
		buf := entry.doExecuteTemplate("apis", doApis(req), logger)
		writer.WriteHeader(http.StatusOK)
		writer.Write(buf.Bytes())
	case "entries":
