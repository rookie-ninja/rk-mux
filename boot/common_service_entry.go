...
const (
	...
	CommonServiceEntryDescription = "Internal RK entry which implements commonly used API with mux framework."
)
...
// @contact.name rk-dev
// @contact.url https://github.com/rookie-ninja/rk-mux
// @contact.email lark@pointgoal.io

// @license.name Apache 2.0 License
// @license.url https://github.com/rookie-ninja/rk-mux/blob/master/LICENSE.txt
...
// 5: Apis Returns list of apis registered in mux router
...
func (entry *CommonServiceEntry) Healthy(writer http.ResponseWriter, req *http.Request) {
	rkmuxinter.WriteJson(writer, http.StatusOK, doHealthy(writer, req))
}
...
func (entry *CommonServiceEntry) Gc(writer http.ResponseWriter, req *http.Request) {
	rkmuxinter.WriteJson(writer, http.StatusOK, doGc(writer, req))
}
...
func (entry *CommonServiceEntry) Info(writer http.ResponseWriter, req *http.Request) {
	rkmuxinter.WriteJson(writer, http.StatusOK, doInfo(writer, req))
}
...
func (entry *CommonServiceEntry) Configs(writer http.ResponseWriter, req *http.Request) {
	rkmuxinter.WriteJson(writer, http.StatusOK, doConfigs(writer, req))
}
...
func (entry *CommonServiceEntry) Sys(writer http.ResponseWriter, req *http.Request) {
	rkmuxinter.WriteJson(writer, http.StatusOK, doSys(writer, req))
}
...
func (entry *CommonServiceEntry) Entries(writer http.ResponseWriter, req *http.Request) {
	rkmuxinter.WriteJson(writer, http.StatusOK, doEntries(writer, req))
}
...
func (entry *CommonServiceEntry) Certs(writer http.ResponseWriter, req *http.Request) {
	rkmuxinter.WriteJson(writer, http.StatusOK, doCerts(writer, req))
}
...
func (entry *CommonServiceEntry) Logs(writer http.ResponseWriter, req *http.Request) {
	rkmuxinter.WriteJson(writer, http.StatusOK, doLogs(writer, req))
}

// Extract mux entry from zap middleware
func getEntry(req *http.Request) *MuxEntry {
	if req == nil {
		return nil
	}

	entryRaw := rkentry.GlobalAppCtx.GetEntry(rkmuxctx.GetEntryName(req))
	if entryRaw == nil {
		return nil
	}
...
func (entry *CommonServiceEntry) Deps(writer http.ResponseWriter, req *http.Request) {
	rkmuxinter.WriteJson(writer, http.StatusOK, doDeps(writer, req))
}
...
func (entry *CommonServiceEntry) License(writer http.ResponseWriter, req *http.Request) {
	rkmuxinter.WriteJson(writer, http.StatusOK, doLicense(writer, req))
}
...
func (entry *CommonServiceEntry) Readme(writer http.ResponseWriter, req *http.Request) {
	rkmuxinter.WriteJson(writer, http.StatusOK, doReadme(writer, req))
}
...
func (entry *CommonServiceEntry) Git(writer http.ResponseWriter, req *http.Request) {
	rkmuxinter.WriteJson(writer, http.StatusOK, doGit(writer, req))
}
...
func constructSwUrl(entry *MuxEntry, req *http.Request) string {
...
func doApis(req *http.Request) *rkentry.ApisResponse {
	res := &rkentry.ApisResponse{
		Entries: make([]*rkentry.ApisResponse_Entry, 0),
	}

	muxEntry := getEntry(req)

	if muxEntry != nil {
		muxEntry.Router.Walk(func(route *mux.Route, router *mux.Router, ancestors []*mux.Route) error {
			...
			entry := &rkentry.ApisResponse_Entry{
				Rest: &rkentry.ApisResponse_Rest{
					...
					Pattern: pathTemp,
					Method:  methods,
					SwUrl:   constructSwUrl(muxEntry, req),
				},
				EntryName: muxEntry.GetName(),
			}
...
func (entry *CommonServiceEntry) Apis(writer http.ResponseWriter, req *http.Request) {
	writer.Header().Set("Access-Control-Allow-Origin", "*")

	rkmuxinter.WriteJson(writer, http.StatusOK, doApis(req))
}
...
func doReq(req *http.Request) *rkentry.ReqResponse {
	metricsSet := rkmuxmetrics.GetServerMetricsSet(req)
	if metricsSet == nil {
	...
	vector := metricsSet.GetSummary(rkmuxmetrics.ElapsedNano)
	if vector == nil {
		return &rkentry.ReqResponse{
			Metrics: make([]*rkentry.ReqMetricsRK, 0),
		}
	...
	muxEntry := GetMuxEntry(rkmuxctx.GetEntryName(req))
	if muxEntry != nil {
		muxEntry.Router.Walk(func(route *mux.Route, router *mux.Router, ancestors []*mux.Route) error {
			pathTemp, _ := route.GetPathTemplate()
...
func (entry *CommonServiceEntry) Req(writer http.ResponseWriter, req *http.Request) {
	rkmuxinter.WriteJson(writer, http.StatusOK, doReq(req))
}
